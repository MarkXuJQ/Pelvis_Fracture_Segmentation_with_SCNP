...
def ensure_preprocessed_dataset_matches_raw(
    ...
    raw_build_manifest = _raw_build_manifest_for_preprocess(raw_dataset_json_path.parent)
    preprocess_manifest_path = Path(preprocessed).resolve() / dataset_name / "fracsegnet_disMap_preprocess.json"
    if not preprocess_manifest_path.is_file():
        raise RuntimeError(
            "Missing stage-2 preprocessing manifest. "
            "Re-run preprocessing with `--preprocess --reset_preprocess` so the anatomy-derived FracSegNet masked-CT build is used."
        )
    preprocess_manifest = json.loads(preprocess_manifest_path.read_text(encoding="utf-8"))
    expected_digest = _raw_build_manifest_digest(raw_build_manifest)
    if (
        preprocess_manifest.get("raw_dataset_build") != raw_build_manifest
        or preprocess_manifest.get("raw_dataset_build_digest") != expected_digest
    ):
        raise RuntimeError(
            "The preprocessed nnU-Net cache was not built from the current anatomy-derived FracSegNet masked-CT raw dataset. "
            "Re-run preprocessing with `--preprocess --reset_preprocess` before training."
        )
...
def _raw_build_manifest_for_preprocess(raw_dataset_dir: Path) -> dict:
    ...
    if not manifest_path.is_file():
        raise RuntimeError(
            "Missing stage-2 raw build manifest. "
            "Rebuild the raw dataset so second-stage images are generated from full CT plus stage-1 anatomy labels."
        )

    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    if manifest.get("dataset_build_schema_version") != DATASET_BUILD_SCHEMA_VERSION:
        raise RuntimeError(
            "The stage-2 raw data was built with an old schema. "
            "Rebuild it so second-stage images are generated from full CT plus stage-1 anatomy labels."
        )
    if manifest.get("image_semantics") != SECOND_STAGE_IMAGE_SEMANTICS:
        raise RuntimeError(
            "The stage-2 raw images are not marked as FracSegNet-style masked CT. "
            f"Got image_semantics={manifest.get('image_semantics')!r}."
        )
    if manifest.get("image_generation_mode") != SECOND_STAGE_IMAGE_GENERATION_MODE:
        raise RuntimeError(
            "The stage-2 raw images were generated by an unexpected mode. "
            f"Got image_generation_mode={manifest.get('image_generation_mode')!r}."
        )
...
def parse_patient_id(case_name: str) -> str:
    ...
    if len(parts) < 3:
        raise RuntimeError(f"Unexpected stage-2 per-bone case name format: {case_name}")
    patient_id = parts[1]
...
def collect_case_names(raw_dataset_dir: Path) -> list[str]:
    ...
    if not case_names:
        raise RuntimeError(f"No stage-2 per-bone training cases found in: {images_tr}")
    return case_names
...
def build_patient_level_splits(case_names: list[str], patient_val_count: int, seed: int) -> tuple[list[dict], dict]:
    ...
    for fold_idx, val_patient_ids in enumerate(folds_patient_ids):
        ...
        if sorted(train_cases + val_cases) != all_case_names:
            raise RuntimeError(f"Fold {fold_idx} does not cover the full stage-2 per-bone case set.")
...
def write_patient_split_artifacts(
    ...
    lines = [
        ...
        f"- Validation stage-2 per-bone cases: {fold_info['val_cases']}",
        f"- Seed: {summary['seed']}",
        "",
        "Validation patient IDs",
        ", ".join(fold_info["val_patient_ids"]),
        "",
        "Validation stage-2 per-bone cases",
    ]
