...
from training.data.build_stage2_masked_ct_dataset import build_raw_dataset
...
def _generate_configuration_dismaps(
    ...
    manifest = {
        ...
        "generator": "FracSegNet official calculate_disMap equivalent (stage-2 main fracture segment=1, non-main fracture segments>=2)",
...
def _raw_build_manifest_for_preprocess(raw_dataset_dir: Path) -> dict:
    manifest_path = raw_dataset_dir / "dataset_build_manifest.json"
    if not manifest_path.is_file():
        raise RuntimeError(f"Missing raw dataset build manifest: {manifest_path}")
...
def _raw_build_manifest_digest(raw_build_manifest: dict) -> str:
    payload = json.dumps(raw_build_manifest, sort_keys=True, separators=(",", ":")).encode("utf-8")
    return hashlib.sha256(payload).hexdigest()


def _preprocessed_manifest_matches_raw_build(raw_dataset_dir: Path, preprocessed_dataset_dir: Path) -> bool:
    manifest_path = preprocessed_dataset_dir / "fracsegnet_disMap_preprocess.json"
    if not manifest_path.is_file():
        return False

    try:
        expected_raw_build = _raw_build_manifest_for_preprocess(raw_dataset_dir)
        expected_digest = _raw_build_manifest_digest(expected_raw_build)
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    except (json.JSONDecodeError, RuntimeError):
        return False

    if expected_raw_build.get("dataset_build_schema_version") != DATASET_BUILD_SCHEMA_VERSION:
        return False
    if expected_raw_build.get("image_semantics") != SECOND_STAGE_IMAGE_SEMANTICS:
        return False
    if expected_raw_build.get("image_generation_mode") != SECOND_STAGE_IMAGE_GENERATION_MODE:
        return False

    return (
        manifest.get("raw_dataset_build") == expected_raw_build
        and manifest.get("raw_dataset_build_digest") == expected_digest
    )
...
def _can_reuse_preprocessed_dataset(
    ...
    if not _raw_and_preprocessed_dataset_json_match(raw_dataset_dir, preprocessed_dataset_dir):
        return False
    if not _preprocessed_manifest_matches_raw_build(raw_dataset_dir, preprocessed_dataset_dir):
        return False
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Build the anatomy-derived FracSegNet-style masked-CT raw dataset, "
            "run nnUNet preprocessing, and generate disMap sidecars."
        )
    )
    ...
    parser.add_argument(
        "--reset_raw",
        action="store_true",
        help="Rebuild the raw stage-2 masked-CT images before preprocessing. By default an up-to-date raw build is reused.",
    )
    ...
    if int(args.task_id) != DATASET_ID:
        raise RuntimeError(f"This project expects nnU-Net dataset id {DATASET_ID}, got {args.task_id}")

    raw_dataset_dir = build_raw_dataset(data_root=project_paths.data_root, reset_existing=bool(args.reset_raw))
    ...
    raw_build_manifest = _raw_build_manifest_for_preprocess(raw_dataset_dir)
        ...
        json.dumps(
            {
                ...
                "raw_dataset_build": raw_build_manifest,
                "raw_dataset_build_digest": _raw_build_manifest_digest(raw_build_manifest),
                "generator": "FracSegNet official calculate_disMap equivalent (stage-2 main fracture segment=1, non-main fracture segments>=2)",
