...
from training.method.dismap_loader import nnUNetDataLoaderWithDisMap
...
class nnUNetTrainerSCNPCTOnlyBase(nnUNetTrainer):
    ...
    def __init__(
        ...
        quick_train_iterations = os.environ.get("NNUNET_SCNP_TRAIN_ITERATIONS_PER_EPOCH")
        quick_val_iterations = os.environ.get("NNUNET_SCNP_VAL_ITERATIONS_PER_EPOCH")
        if quick_train_iterations:
            self.num_iterations_per_epoch = int(quick_train_iterations)
        if quick_val_iterations:
            self.num_val_iterations_per_epoch = int(quick_val_iterations)
    ...
    def _should_log_grouped_secondary_pseudo_dice(dataset_json: dict) -> bool:
        ...
        if not secondary_label_names:
            return False
        return all(
            name in {"other_fragments", "secondary_fragments", "segment_2"}
    ...
    def initialize(self):
        ...
        if not self.is_smoke_test and (
            os.environ.get("NNUNET_SCNP_TRAIN_ITERATIONS_PER_EPOCH")
            or os.environ.get("NNUNET_SCNP_VAL_ITERATIONS_PER_EPOCH")
        ):
            self.print_to_log_file(
                "Short-run iteration limits enabled:",
                f"num_iterations_per_epoch={self.num_iterations_per_epoch},",
                f"num_val_iterations_per_epoch={self.num_val_iterations_per_epoch},",
                f"disable_checkpointing={self.disable_checkpointing}",
            )
        ...
        if self.log_grouped_secondary_pseudo_dice:
            self.print_to_log_file(
                "Primary validation pseudo Dice follows the stage-2 three-class label space; "
                "an additional grouped metric will also be logged as background / main fracture segment / secondary fragments."
            )
