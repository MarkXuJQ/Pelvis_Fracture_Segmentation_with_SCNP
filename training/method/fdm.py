...
def _as_spatial_label(label: np.ndarray) -> np.ndarray:
    label = np.asarray(label)
    if label.ndim == 4:
        if label.shape[0] != 1:
            raise RuntimeError(f"Expected a single-channel label with shape [1, ...], got {label.shape}")
        return label[0]
    if label.ndim == 3:
        return label
    raise RuntimeError(f"Expected a 3D label or a single-channel 4D label, got {label.shape}")


def calculate_dis_map(label: np.ndarray) -> np.ndarray:
    seg_label = _as_spatial_label(label)
    if np.unique(seg_label).max() > 1:
    ...
    return (np.ones_like(seg_label[None], dtype=np.float32) * 0.2).astype(np.float32, copy=False)
