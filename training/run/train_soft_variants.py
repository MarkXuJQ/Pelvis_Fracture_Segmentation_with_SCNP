...
def main(
    *,
    task_dir: Path,
    default_trainer: str,
    description: str = "Train a CT-only soft-variant SCNP model on the stage-2 masked-CT training dataset.",
) -> None:
    ...
    parser.add_argument("--scnp_rf", type=int, default=3)
    ...
    parser.add_argument("--scnp_fdm_threshold", type=float, default=0.5)
    parser.add_argument("--scnp_fdm_power", type=float, default=1.0)
    parser.add_argument("--scnp_fdm_beta", type=float, default=10.0)
    parser.add_argument("--scnp_soft_tau", type=float, default=1.0)
    ...
    prepare_flat_results_layout(
        ...
        trainer_name=str(args.trainer_name),
    ...
    os.environ["NNUNET_SCNP_FDM_BETA"] = str(float(args.scnp_fdm_beta))
    os.environ["NNUNET_SCNP_SOFT_TAU"] = str(float(args.scnp_soft_tau))
