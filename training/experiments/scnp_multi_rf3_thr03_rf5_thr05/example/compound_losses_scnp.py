...
class SCNPCEDiceMultiRF(nn.Module):
    """
    CE + Dice over SCNP-penalized logits with FDM-dependent receptive fields.

    FDM gating rule (after per-sample min-max normalization):
      - fdm < fdm_low: no SCNP, use raw logits
      - fdm_low <= fdm <= fdm_high: SCNP with receptive_field_low
      - fdm > fdm_high: SCNP with receptive_field_high
    """
    ...
    def __init__(
        ...
        ignore_label=None,
        dice_class=None,
        receptive_field_low: int = 3,
        receptive_field_high: int = 5,
        kappa: float = 9999.0,
        joint_standard_weight: float = 0.0,
        fdm_low: float = 0.3,
        fdm_high: float = 0.5,
        ...
        self.weight_dice = float(weight_dice)
        self.ignore_label = ignore_label
        self.receptive_field_low = int(receptive_field_low)
        self.receptive_field_high = int(receptive_field_high)
        self.kappa = float(kappa)
        self.joint_standard_weight = float(joint_standard_weight)
        self.fdm_low = float(fdm_low)
        self.fdm_high = float(fdm_high)
        self.fdm_power = float(fdm_power)

        if not (0.0 <= self.fdm_low <= 1.0 and 0.0 <= self.fdm_high <= 1.0):
            raise ValueError(f"fdm_low/high must be in [0,1], got {self.fdm_low}/{self.fdm_high}")
        if self.fdm_high < self.fdm_low:
            raise ValueError(f"fdm_high must be >= fdm_low, got {self.fdm_high} < {self.fdm_low}")
    ...
    def _scnp_logits(
        ...
        # SCNP is applied channel-wise with same-size pooling (stride=1, symmetric padding).
        # 2D and 3D are both supported because nnUNet can run in either mode.
        max_pool, kernel_size = _get_pool_params(logits.ndim, int(receptive_field))
        pad = tuple(k // 2 for k in kernel_size)
        stride = tuple(1 for _ in kernel_size)
    ...
    def _normalize_fdm(self, fdm: torch.Tensor, logits: torch.Tensor) -> torch.Tensor:
        if fdm.dim() == (logits.dim() - 1):
            fdm = fdm.unsqueeze(1)
        if fdm.dim() != logits.dim():
            raise RuntimeError(
    ...
    def _prepare_fdm_masks(
        ...
        fdm_norm = self._normalize_fdm(fdm, logits)
        mask_mid = (fdm_norm >= self.fdm_low) & (fdm_norm <= self.fdm_high)
        mask_high = fdm_norm > self.fdm_high

        valid = valid_mask.unsqueeze(1).float()
        return mask_mid.float() * valid, mask_high.float() * valid
    ...
    def forward(
        ...
        scnp_low = self._scnp_logits(net_output, target_onehot, valid_mask, self.receptive_field_low)
        if fdm is None:
            scnp_logits = scnp_low
        else:
            scnp_high = self._scnp_logits(net_output, target_onehot, valid_mask, self.receptive_field_high)
            mask_mid, mask_high = self._prepare_fdm_masks(fdm, net_output, valid_mask)
            if mask_mid is None or mask_high is None:
                scnp_logits = scnp_low
            else:
                mask_other = (1.0 - mask_mid - mask_high).clamp_min(0.0)
                scnp_logits = mask_mid * scnp_low + mask_high * scnp_high + mask_other * net_output
