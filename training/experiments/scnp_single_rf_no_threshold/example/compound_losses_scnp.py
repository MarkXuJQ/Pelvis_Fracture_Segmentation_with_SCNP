...
class SCNPCEDice(nn.Module):
    ...
    def __init__(
        ...
        self.use_hard_threshold = bool(use_hard_threshold)
    ...
    def _prepare_fdm_mask(
        ...
        # Optional shape control before the final FDM-guided SCNP mixing.
        if self.fdm_power != 1.0:
            mask = mask.pow(self.fdm_power)
        if self.use_hard_threshold:
            # Thresholded gate: FDM only decides where SCNP is enabled.
            mask = (mask >= self.fdm_threshold).float()
        # Otherwise keep the continuous normalized FDM as a soft mixing weight.
...
class SCNPCEDiceNoThreshold(SCNPCEDice):
    """
    SCNP CE+Dice with continuous FDM-guided blending.

    Compared with the original thresholded variant, the normalized FDM map is
    used directly as the interpolation weight between SCNP logits and raw
    logits, so there is no hard binary gate.
    """

    def __init__(self, *args, **kwargs):
        kwargs.pop("use_hard_threshold", None)
        super().__init__(*args, use_hard_threshold=False, **kwargs)
